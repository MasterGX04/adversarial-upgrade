...
def compute_masking_loss(audio, delta, sr=16000, n_fft=2048, hop_length=512):
    """
    Compute imperceptibility loss: penalize delta where it exceeds masking threshold.
    
    audio: np.ndarray of original waveform
    delta: torch.Tensor [time], perturbation signal
    sr: sample rate
    """
    
    # 1. Compute masking threshold from original audio
    theta_xs, _ = generate_th(audio, sr) # shape [freq, time] (Librosa STFT)
    theta_xs = torch.tensor(theta_xs.T, dtype=torch.float32).to(delta.device)
    
    # 2. STFT of delta (perturbation)
    delta_stft = torch.stft(delta, n_fft=n_fft, hop_length=hop_length, return_complex=True)
    delta_mag = delta_stft.abs() # shape: [freq_bins, time_steps]
    
    # 3. Match dimensions (clip or pad threshold to delta_mag)
    min_t = min(delta_mag.shape[1], theta_xs.shape[1])
    min_f = min(delta_mag.shape[0], theta_xs.shape[0])
    delta_mag = delta_mag[:min_f, :min_t]
    threshold = theta_xs[:min_f, :min_t]
    
    # 4. Loss = mean over bins where delta exceeds masking threshold
    loss = torch.mean(torch.relu(delta_mag - threshold))
    return loss